...
public class InputView implements Closeable {
    ...
    public List<LadderResult> readLadderResults(int requiredLength) {
        System.out.printf(Q_NAMES_OF_RESULTS, config.getNameLength());
        String[] names = readNames(requiredLength);
        return IntStream.range(0,names.length)
    ...
    private String[] readNames(int requiredLength) {
        try {
            ...
            System.out.println(e.getMessage());
            return readNames(requiredLength);
        }
    }
